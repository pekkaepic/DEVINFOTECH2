...
function initNavigation() {
  ...
  if (mainNav && !mainNav.querySelector(".drawer-header")) {
    const bName = typeof SITE_CONFIG !== "undefined" ? SITE_CONFIG.businessName : "DevInfoTech";
  ...
  document.querySelectorAll(".brand-logo-slot").forEach((container) => {
    ...
    if (SITE_CONFIG.logo.type === "image" && SITE_CONFIG.logo.image) {
      ...
      img.onerror = function() {
        ...
        this.outerHTML = `<div class="brand-text-wrapper"><span class="brand-name">${SITE_CONFIG.businessName}</span><span class="brand-tagline">${SITE_CONFIG.logo.tagline || ""}</span></div>`;
      };
      ...
      wrapper.innerHTML = `
        <span class="brand-name">${SITE_CONFIG.logo.text || SITE_CONFIG.businessName}</span>
        ${SITE_CONFIG.logo.tagline ? `<span class="brand-tagline">${SITE_CONFIG.logo.tagline}</span>` : ""}
      `;
